<template>
  <main class="view">
    ...
    <vuetable
      ref="vuetable"
      :fields="['id', 'name', {name: 'race_name', title: 'Race'}, 'hp', 'actions']"
      :api-mode="false"
      :data="data"
      :css="css.table"
      class="table-wrapper"
...
import EditCreatureForm from "../components/forms/EditCreature";
...
export default {
  ...
  methods: {
    onActionDelete(action, data) {
      http.delete(`/creature/${data.id}`).then(() => this.updateTable());
    },
    handleSubmit(data) {
    ...
    addItem(data) {
      http.post("/creature", data).then(() => this.updateTable());
    },
    editItem(data) {
      http
        .patch(`/creature/${this.action.currentId}`, data)
